...
function createStatusBar(screen: blessed.Widgets.Screen): blessed.Widgets.BoxElement {
...
function createChatBox(screen: blessed.Widgets.Screen): blessed.Widgets.BoxElement {
  return blessed.box({
    parent: screen,
    top: 3,
    left: 0,
...
function createInputBox(screen: blessed.Widgets.Screen): blessed.Widgets.TextareaElement {
...
function createPlaceholderLabel(screen: blessed.Widgets.Screen): blessed.Widgets.BoxElement {
  return blessed.box({
    parent: screen,
    bottom: 1,
    left: 2,
...
function formatMessage(msg: Message): string {
  ...
  if (msg.role === "user") {
    ...
    const rendered = marked(msg.content) as string;
    return `\n${chalk.bold.green("┌─ Claude" + modelName + " ─────────────────────────────")}\n${rendered}\n${chalk.dim(time)}\n`;
  }
}

function updateChatBox(chatBox: blessed.Widgets.BoxElement, streamingContent?: string): void {
  let content = "";
...
function updatePlaceholder(placeholder: blessed.Widgets.BoxElement, text: string): void {
...
function showSettings(screen: blessed.Widgets.Screen, chatBox: blessed.Widgets.BoxElement, statusBar: blessed.Widgets.BoxElement): void {
  const config = getConfig();
...
async function sendMessage(
  ...
  placeholder: blessed.Widgets.BoxElement,
  statusBar: blessed.Widgets.BoxElement
): Promise<void> {
  if (!isConfigured()) {
